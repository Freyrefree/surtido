...
while($row = mysql_fetch_array($QueryComision)) {
    ...
    while($row2 = mysql_fetch_array($QueryEmpresa)) {
        ...
        /*$QueryEmpleado=mysql_query("SELECT * FROM usuarios WHERE tipo != 'a' AND id_sucursal = '$id_sucursal'");*/
        ...
        #fin valores de raparaciones obtener datos
    ...
    $objPHPExcel->setActiveSheetIndex(0)
            ...
            ->setCellValue('G'.$i,"-")
            ->setCellValue('H'.$i,"$ ".number_format($totales_comision,2))
            ->setCellValue('I'.$i,"$ ".number_format($totales_utilidad-$totales_comision,2));
    $i++;
    $objPHPExcel->setActiveSheetIndex(0)
            ->setCellValue('A'.$i,"")
            ->setCellValue('B'.$i,"")
...
header('Content-Disposition: attachment;filename="ReporteGeneralVentas.xlsx"');
