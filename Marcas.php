...
<table width="80%" border="0" class="table">
  <tr class="info">
    <td><center><strong>Administrar Marcas</strong></center></td>
  </tr>
  <td>
<table width="80%" border="0">
  <tr>
    ...
    <table width="80%" border="0" class="table">
      ...
      <?php 
		$can=mysql_query("SELECT * FROM marca");
    ...
    <?php 
		if(empty($_GET['codigo'])){
			$can=mysql_query("SELECT MAX(id_marca) as numero FROM marca");
			...
			$can=mysql_query("SELECT * FROM marca WHERE id_marca=$s_codigo");
			if($dato=mysql_fetch_array($can)){
	...
	if(!empty($_POST['s_nombre'])){
		...
		$can=mysql_query("SELECT * FROM marca WHERE id_marca=$ss_codigo");
		if($dato=mysql_fetch_array($can)){
		//actualizar seccion
			$xSQL="Update marca Set nombre='$ss_nombre' Where id_marca=$ss_codigo";
			mysql_query($xSQL);
			echo '	<div class="alert alert-success">
						  <button type="button" class="close" data-dismiss="alert">X</button>
						  <strong>Marca!</strong> Actualizado con Exito <a href="Marcas.php">[Clic Para Actualizar]</a>
					</div>';
		}else{
		//guardar seccion
			$sql="INSERT INTO marca (nombre, estado) VALUES ('$ss_nombre','s')";
				mysql_query($sql);
			echo '	<div class="alert alert-success">
						  <button type="button" class="close" data-dismiss="alert">X</button>
						  <strong>Marca!</strong> Guardado con Exito <a href="Marcas.php">[Clic Para Actualizar]</a>
					</div>';
		}
	}
	?>
