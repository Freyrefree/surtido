<?php
function PDFA($idCredito,$abono,$resto,$cambio,$denominacion,$dinero_recibido)
{
      //session_start();
      include('host.php');
      ...
      {
          ## Datos Cajera/O ##
          $consultaCajero     = "SELECT nom FROM usuarios WHERE usu = '$usuario' AND id_sucursal = '$idSucursal'";
          ...
          if($paqueteDetalle     = consultar($con,$consultaDetalle)){
              ...
              while($datoDetalle = mysqli_fetch_array($paqueteDetalle)){
                ...
                if($iccid != ""){
                  $addICCID = " <br> ICCID: ".saltoCadena2($iccid);
                }else{
                   $addICCID = "";
                }
  
                if($imei != ""){
                  $addIMEI = " <br> IMEI: ".saltoCadena2($imei);
                }else{                
                   $addIMEI = "";
                }
                ########################################
                ...
                <tr>
                  ...
                  <td class="product" align="left">'.saltoCadena2($nombreProducto).$addICCID.$addIMEI.'</td>
              ...
              $consultaProd = "SELECT * FROM creditopago WHERE idCredito = '$idCredito'";
              ...
              if($paqueteProd = consultar($con,$consultaProd)){
                ...
                <table class="nota1">
                  <tr>
                    <td class="product"><center>#</center></td>
                    <td class="product"><center>ABONO</center></td>
                    <td class="product"><center>FECHA</center></td>
                  </tr>';
                  $contador = 0;
                  ...
                  while($dato = mysqli_fetch_array($paqueteProd)){
                      ...
                      $codigoHTML.='<tr>';
                          ...
                          $codigoHTML.='<td class="product">'.$contador.'</td>';  
                          $codigoHTML.='<td class="product">$ '.number_format((float)$dato['abono'], 2, '.', '').'</td>';
                          $codigoHTML.='<td class="product">'.$dato['fechaPago'].'</td> ';  
                  ...
                  $codigoHTML.='<tr>';
                    ...
                    $codigoHTML.='<td class="product">TOTAL</td>';
                    $codigoHTML.='<td colspan="2" class="product">$'.number_format((float)$dinero_recibido, 2, '.', '').'</td>';  
              ...
              <td align="right" ><strong>'.number_format($resto,2).'<strong></td>
              </tr>';
          ...
          $mpdf->Output('Facturas/ABONO'."_".$idCredito."_".$idSucursal.'.pdf');
      }
  }
  
  
  function saltoCadena2($stirng){
