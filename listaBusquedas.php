            ...
            <?php 
             ...
             <input name="busqueda" type="text" class="span2" size="60" list="characters" placeholder="Buscar" autocomplete="off">
              ...
              <?php
                ...
                $can=mysql_query("SELECT * FROM registrobusquedas ORDER BY id DESC");
...
<table width="80%" border="0" class="table">
  <tr class="info">
    <td colspan="7"><center><strong>Historial de Consultas</strong></center></td>
  </tr>
  <tr>
    <td width="5%" ><strong>No. Reparación</strong></td>
    <td width="15%"><strong>Busqueda</strong></td>
    <td width="15%"><strong>Equipo</strong></td>
    ...
    if(empty($_POST['busqueda'])){
        ...
        $can=mysql_query("SELECT * FROM registrobusquedas
                            WHERE   usuario       LIKE '%$buscar%' OR  
                                    cliente       LIKE '%$buscar%' OR 
                                    id_reparacion LIKE '%$buscar%'
                            ORDER BY id DESC");
    }

    while($dato=mysql_fetch_array($can)){
        $id_repara = $dato['id_reparacion'];
        $equipo = "";
        $can2=mysql_query("SELECT marca, modelo FROM reparacion WHERE id_reparacion = $id_repara");
        if($dato2=mysql_fetch_array($can2)){
            $equipo = $dato2['marca']. " " .$dato2['modelo'];
        }
    ?>
        
        <tr>
            <td><?php echo $id_repara; ?></td>
