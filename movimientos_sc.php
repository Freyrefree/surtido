  ...
  <tr>
    ...
    if(empty($_POST['inicio']) && empty($_POST['fin'])){
        if ($_SESSION['tipo_usu'] == "a") {
            $can=mysql_query("SELECT * FROM movimiento ORDER BY fecha DESC, id_movimiento DESC");
        }else{
            $can=mysql_query("SELECT * FROM movimiento WHERE id_suc_entrada = '$id_sucursal' AND estado = '1' ORDER BY fecha DESC, id_movimiento DESC");
        }
    }else{
        $datestart  = $_POST['inicio'];
        $datefinish = $_POST['fin'];
        if ($_SESSION['tipo_usu'] == "a") {
            $can=mysql_query("SELECT * FROM movimiento where fecha BETWEEN '".$datestart."' AND '".$datefinish."'");
        }else {
            $can=mysql_query("SELECT * FROM movimiento where fecha BETWEEN '".$datestart."' AND '".$datefinish."' AND id_sucursal = '$id_sucursal' AND estado = '1'");
        }
    }
